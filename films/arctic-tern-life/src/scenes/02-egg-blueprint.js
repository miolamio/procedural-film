...
//   1 navyDeep, then the blueprint plate (navy, 60 px grid) fading in on T 2.0
//   2 guide geometry: circles r 470 / 640 at (540, 900) turning 6 degrees, corner diagonals, construction lines
//   3 shell-wall section band y 432 to 556 (cuticle, palisade, pore canals, mammillary cones, membranes)
//   4 ground band y 1178 to 1220 with pebble cells, a faint shingle section below y 1560
//   5 measurement: length bracket y 1260, height bracket x 100, tick scale x 1020, division counter plate
//   6 the G1 egg: grid calm, shade stipple and contour hatching, pore lattice, albumen, chalazae,
//     yolk (layers, latebra, stipple), air cell, blotch outlines in schemEgg, double outline
//   7 blastoderm and nuclei dividing 1, 2, 4, 8; the curled embryo with its vessels and egg tooth
//   8 spark, magenta events, network insets (blotch pigment, pore net), the canonical cycle ring
//
// Shot 17 redraws this egg's final state: the egg drawing lives in the named functions
// drawEggOutline, drawPoreLattice, drawBlotchOutlines, drawAirCell, drawYolk, drawNucleus (plus eggGeo).
(function () {
  'use strict';
  ...
  // blastoderm disc on the yolk top: an ellipse of tissue cells, alpha a
  function drawBlastoderm(ctx, L, a, bi) {
    if (a <= 0) return;
    const P = L.pal;
    const pts = L.ellipsePts(HOME[0], HOME[1] + 2, 92, 32, 48);
    ctx.save();
    ctx.globalAlpha *= a;
    fill(ctx, (() => {
      const p = new Path2D();
      L.tracePath(p, pts, true);
      return p;
    })(), P.navyLight, 0.5);
    L.hexLattice(ctx, pts, { r: 8, alpha: 0.32, width: 1, seed: sd('bd'), boil: bi });
    const o = new Path2D();
    wob(L, o, pts, sd('bd-out'), 0.5, bi, true);
    stroke(ctx, o, P.lavender, 0.6, 1.3);
    ctx.restore();
  }

  // the curled embryo: navy body, lavender outline, somites, limb buds, big eye, bill and egg tooth
  function drawEmbryo(ctx, L, g, k, bi) {
    if (k <= 0) return;
    const P = L.pal, lav = P.lavender, white = P.lineWhite;
    const em = g.em;
    const kc = clamp(k);
    ctx.save();
    // condensing: the body gathers in from the blastoderm, scaled about the disc centre
    const s = lerp(0.62, 1, kc);
    ctx.translate(HOME[0] - 40, HOME[1] + 30);
    ctx.scale(s, s);
    ctx.translate(-(HOME[0] - 40), -(HOME[1] + 30));
    ctx.globalAlpha *= Math.pow(kc, 0.6);

    // vitelline vessels over the yolk (drawn first, under the body)
    const vp = [new Path2D(), new Path2D(), new Path2D()];
    em.vessels.forEach((v, i) => {
      const n = Math.max(2, Math.round(v.pts.length * clamp(kc * 1.2 - v.d * 0.1)));
      wob(L, vp[v.d], v.pts.slice(0, n), sd('ves', i), 0.5, bi, false);
    });
    ctx.save();
    const yc = new Path2D();
    yc.arc(YOLK[0], YOLK[1], YOLK[2] - 3, 0, TAU);
    ctx.clip(yc);
    stroke(ctx, vp[0], lav, 0.5, 1.4);
    stroke(ctx, vp[1], lav, 0.38, 1.1);
    stroke(ctx, vp[2], lav, 0.28, 1);
    // sinus terminalis: the ring vein bounding the vessel field
    const st = new Path2D();
    wob(L, st, L.ellipsePts(470, 800, 190, 128, 72, -0.12), sd('sinus'), 0.8, bi, true);
    stroke(ctx, st, lav, 0.22 * kc, 1, [6, 5]);
    ctx.restore();

    const body = new Path2D();
    wob(L, body, em.body, sd('emb'), 0.6, bi, true);
    const head = new Path2D();
    wob(L, head, L.ellipsePts(em.head[0], em.head[1], em.head[2], em.head[2] * 0.94, 40, 0.3), sd('head'), 0.6, bi, true);
    const buds = new Path2D();
    wob(L, buds, em.wing, sd('wing'), 0.4, bi, true);
    wob(L, buds, em.leg, sd('leg'), 0.4, bi, true);
    const bill = new Path2D();
    wob(L, bill, em.bill, sd('bill'), 0.3, bi, true);
    for (const p of [body, head, buds, bill]) fill(ctx, p, P.navy, 0.9);
    fill(ctx, body, P.navyLight, 0.35);
    fill(ctx, head, P.navyLight, 0.35);
    // contour hatching on the shadow (ventral, lower right) half of the body
    const hh = new Path2D();
    for (let i = 0; i < em.somites.length; i++) {
      const q = em.somites[i];
      const w = q.w * 0.85;
      const a = [q.p[0] + q.n[0] * w * 0.2, q.p[1] + q.n[1] * w * 0.2];
      const b = [q.p[0] + q.n[0] * w, q.p[1] + q.n[1] * w];
      hh.moveTo(a[0] + q.n[2] * 5, a[1] + q.n[3] * 5);
      hh.quadraticCurveTo((a[0] + b[0]) / 2 + q.n[2] * 3, (a[1] + b[1]) / 2 + q.n[3] * 3, b[0] + q.n[2] * 5, b[1] + q.n[3] * 5);
    }
    stroke(ctx, hh, lav, 0.25, 1);
    stroke(ctx, buds, lav, 0.7, 1.3);
    stroke(ctx, body, lav, 0.8, 1.5);
    stroke(ctx, head, lav, 0.8, 1.5);
    stroke(ctx, bill, lav, 0.8, 1.3);
    // somite ticks across the dorsal half, and the neural line
    const sm = new Path2D();
    for (const q of em.somites) {
      const w = q.w * 0.45, r = Math.max(1.6, q.w * 0.14);
      const x = q.p[0] - q.n[0] * w, y = q.p[1] - q.n[1] * w;
      sm.moveTo(x + r, y);
      sm.ellipse(x, y, r * 1.3, r, Math.atan2(q.n[3], q.n[2]), 0, TAU);
    }
    stroke(ctx, sm, lav, 0.55, 1);
    const nl = new Path2D();
    const npts = [];
    for (let s2 = em.sLen * 0.04; s2 < em.sLen * 0.93; s2 += 8) {
      const q = em.at(s2);
      const w = em.width(s2) * 0.1;
      npts.push([q.p[0] - q.n[0] * w, q.p[1] - q.n[1] * w]);
    }
    wob(L, nl, npts, sd('neural'), 0.4, bi, false);
    stroke(ctx, nl, white, 0.28, 1, [5, 5]);
    // the heart: a small bulge on the ventral side with a glow
    const hr = new Path2D();
    hr.arc(em.heart[0], em.heart[1], 9, 0, TAU);
    fill(ctx, hr, P.navy, 0.9);
    stroke(ctx, hr, lav, 0.7, 1.2);
    L.glowDot(ctx, em.heart[0], em.heart[1], 3.5, { rays: 0, glow: 5, intensity: 0.8 * kc, seed: 71, boil: bi });
    // eye: a large dark eye, lens and a glint
    const eye = new Path2D();
    eye.arc(em.eye[0], em.eye[1], em.eye[2], 0, TAU);
    fill(ctx, eye, P.navyDeep, 0.9);
    stroke(ctx, eye, lav, 0.85, 1.4);
    const lens = new Path2D();
    lens.arc(em.eye[0] - 2, em.eye[1] + 1, 8, 0, TAU);
    fill(ctx, lens, lav, 0.45);
    const gl = new Path2D();
    gl.arc(em.eye[0] - 6, em.eye[1] - 5, 2.6, 0, TAU);
    fill(ctx, gl, white, 0.95);
    // head vesicles: faint arcs of the forebrain and midbrain
    const ves = new Path2D();
    ves.arc(em.head[0] + 6, em.head[1] - 2, 30, -2.4, -0.4);
    ves.moveTo(em.head[0] + 32, em.head[1] - 20);
    ves.arc(em.head[0] + 14, em.head[1] - 6, 24, -0.9, 0.6);
    stroke(ctx, ves, lav, 0.3, 1, [3, 4]);
    // limb-bud digit lines
    const dg = new Path2D();
    for (const bud of [em.wing, em.leg]) {
      const m = bud[6], a = bud[0], b = bud[12];
      const c = [(a[0] + b[0]) / 2, (a[1] + b[1]) / 2];
      for (const u of [-0.3, 0, 0.3]) {
        dg.moveTo(c[0] + (b[0] - a[0]) * u * 0.6, c[1] + (b[1] - a[1]) * u * 0.6);
        dg.lineTo(lerp(c[0], m[0], 0.75) + (b[0] - a[0]) * u * 0.4, lerp(c[1], m[1], 0.75) + (b[1] - a[1]) * u * 0.4);
      }
    }
    stroke(ctx, dg, lav, 0.35, 1);
    ctx.restore();
    // egg tooth: a lineWhite point on the bill tip, screen-fixed at (250, 860) once condensed
    if (kc >= 1) {
      L.glowDot(ctx, TOOTH[0], TOOTH[1], 3.4, { rays: 4, rayLen: 2.4, glow: 5, intensity: 1.1, seed: 72, boil: bi, rot: (bi % 2) * 45 * DEG });
      const td = new Path2D();
      td.arc(TOOTH[0], TOOTH[1], 3, 0, TAU);
      fill(ctx, td, white, 1);
    }
  }
  ...
  FILM.scene({
    ...
    draw(ctx, tIn, info) {
      ...
      const B_BIRTH = B_BUILD + 6 * FR; // T 2.25, the spark has settled into the nucleus
      const B_LAT = 1.0; // T 2.5
      const B_DIV2 = 1.5; // T 3.0
      const B_DIV4 = 1.75; // T 3.25
      const B_DIV8 = 2.0; // T 3.5
      const B_EMB = 2.25; // T 3.75
      ...
      if (t < B_BUILD) {
        for (let k = 0; k < 2; k++) {
          ...
          ctx.save();
          ctx.beginPath();
          ctx.arc(520, 700, 16 + (k ? 170 : 300) * E.outExpo(u), 0, TAU);
          ctx.strokeStyle = lav;
      ...
      // ---- 3 shell-wall section band (with the lattice sweep) ----
      const kLat = hit(B_LAT, 6, E.outCubic, 0.6);
      drawShellSection(ctx, L, g, hit(B_LAT, 6, E.outExpo, 0.6), bi);

      // ---- 4 ground band ----
      drawGround(ctx, L, g, hit(B_BUILD, 6, E.outExpo, 0.6), bi);

      // ---- 5 measurement ----
      const kLen = hit(B_LAT + 0.375, 6, E.outExpo); // T 2.875
      const kHgt = hit(B_DIV2, 6, E.outExpo); // T 3.0
      const kScale = hit(B_LAT, 12, E.outCubic);
      if (kScale > 0) {
        L.ticks(ctx, 1020, 120, { length: 1680, angle: Math.PI / 2, n: 42, len: 10, major: 5, majorLen: 22, side: 1, alpha: 0.5, width: 1.5, p: kScale, color: lav });
      }
      if (kLen > 0) {
        L.bracket(ctx, 160, 1260, 920, 1260, { p: kLen, alpha: 0.6, cap: 16 });
        L.ticks(ctx, 160, 1260, { length: 760, angle: 0, n: 10, len: 12, side: -1, baseline: false, alpha: 0.6, width: 1.5, color: lav, p: kLen });
        const ext = new Path2D();
        ext.moveTo(160, 912);
        ext.lineTo(160, 1274);
        ext.moveTo(920, 912);
        ext.lineTo(920, 1274);
        stroke(ctx, ext, lav, 0.26 * kLen, 1, [4, 6]);
      }
      if (kHgt > 0) {
        L.bracket(ctx, 100, 622, 100, 1178, { p: kHgt, alpha: 0.6, cap: 16 });
        L.ticks(ctx, 100, 622, { length: 556, angle: Math.PI / 2, n: 8, len: 10, side: -1, baseline: false, alpha: 0.55, width: 1.5, color: lav, p: kHgt });
        const ext = new Path2D();
        ext.moveTo(86, 622);
        ext.lineTo(452, 622);
        ext.moveTo(86, 1178);
        ext.lineTo(300, 1178);
        stroke(ctx, ext, lav, 0.24 * kHgt, 1, [4, 6]);
      }

      // ---- 6 the egg ----------------------------------------------------------------
      const kOut = hit(B_BUILD, 6, E.outExpo, 0.5);
      const kIn = hit(B_BUILD + FR, 6, E.outExpo, 0.5);
      const kFill = hit(B_BUILD + 3 * FR, 4, E.inOutSine);
      if (kFill > 0) {
        // calm the grid inside the shell, then a soft light round the blastodisc
        ctx.save();
        ctx.fillStyle = P.navy;
      ...
      const front = 160 + 780 * kLat;
      const kYolk = hit(B_LAT, 6, E.outExpo, 0.6);
      if (kLat > 0) drawEggShade(ctx, L, g, front, bi);
      drawYolk(ctx, L, g, kYolk, bi);
      if (kLat > 0) {
        drawPoreLattice(ctx, L, g, front, bi);
        if (kLat < 1) {
          // scan line at the sweep front
          const sh = halfH(Math.min(front, 919));
          const sl = new Path2D();
          sl.moveTo(front, 900 - sh - 30);
          sl.lineTo(front, 900 + sh + 30);
          stroke(ctx, sl, white, 0.55 * (1 - kLat), 1.5);
        }
      }
      drawAirCell(ctx, L, g, kYolk, bi);
      // blotch outlines pop on 16ths, blunt end first
      const grpK = [0, 1, 2, 3].map((q) => hit(B_LAT + q * 0.125, 3, E.outBack));
      drawBlotchOutlines(ctx, L, g, grpK, bi);
      drawEggOutline(ctx, L, g, kOut, kIn, bi);

      // ---- 7 blastoderm, nuclei, embryo ----------------------------------------------
      const kCond = t < B_EMB ? 0 : [0.45, 0.8, 1][Math.min(2, drawing(B_EMB))];
      const kDisc = hit(B_DIV8, 6, E.outCubic) * (1 - kCond);
      drawBlastoderm(ctx, L, kDisc, bi);
      drawEmbryo(ctx, L, g, kCond, bi);

      const nuclei = []; // [x, y, scale]
      if (t >= B_BIRTH) {
        const two = [[503, 760], [537, 760]];
        const four = [[486, 750], [500, 776], [540, 744], [554, 770]];
        const ell = (k) => {
          const a = (22.5 + 45 * k) * DEG;
          return [HOME[0] + Math.cos(a) * 62, HOME[1] + 2 + Math.sin(a) * 22];
        };
        // parent (index in four) of each ellipse slot
        const PARENT = [3, 3, 1, 1, 0, 0, 2, 2];
        if (t < B_DIV2) {
          nuclei.push([HOME[0], HOME[1], 1]);
        } else if (t < B_DIV4) {
          const k = hit(B_DIV2, 3, E.outBack);
          for (const p of two) nuclei.push([lerp(HOME[0], p[0], k), lerp(HOME[1], p[1], k), 0.85]);
        } else if (t < B_DIV8) {
          const k = hit(B_DIV4, 3, E.outBack);
          four.forEach((p, i) => {
            const par = two[i < 2 ? 0 : 1];
            nuclei.push([lerp(par[0], p[0], k), lerp(par[1], p[1], k), 0.7]);
          });
        } else {
          const k = popTwos(B_DIV8);
          for (let s = 0; s < 8; s++) {
            const par = four[PARENT[s]], e = ell(s);
            let x = lerp(par[0], e[0], k), y = lerp(par[1], e[1], k);
            let sc = 0.55;
            if (kCond > 0) {
              const q = g.em.slots[s];
              x = lerp(x, q[0], kCond);
              y = lerp(y, q[1], kCond);
              sc = lerp(0.55, 0.3, kCond);
            }
            nuclei.push([x, y, sc]);
          }
        }
      }
      nuclei.forEach(([x, y, s], i) => {
        drawNucleus(ctx, L, x, y, s, {
          bi,
          seed: 60 + i,
          ticks: nuclei.length >= 8 ? 8 : 12,
          tickAlpha: 0.7 * (1 - kCond),
          intensity: lerp(1.25, 0.6, kCond),
          rot: nuclei.length >= 8 ? (i % 2) * 22.5 * DEG : 0,
        });
      });
      // spindle between fresh daughters
      const spindle = (a, pairs) => {
        const sa = 1 - hit(a, 6);
        if (sa <= 0) return;
        const sp = new Path2D();
        for (const [i, j] of pairs) {
          if (!nuclei[i] || !nuclei[j]) continue;
          sp.moveTo(nuclei[i][0], nuclei[i][1]);
          sp.lineTo(nuclei[j][0], nuclei[j][1]);
        }
        stroke(ctx, sp, white, 0.5 * sa, 1.2, [3, 4]);
      };
      if (t >= B_DIV2 && t < B_DIV4) spindle(B_DIV2, [[0, 1]]);
      else if (t >= B_DIV4 && t < B_DIV8) spindle(B_DIV4, [[0, 1], [2, 3]]);
      else if (t >= B_DIV8 && t < B_EMB) spindle(B_DIV8, [[0, 1], [2, 3], [4, 5], [6, 7]]);

      // ---- 8 spark and magenta events ---------------------------------------------------
      // the spark settles from (520, 700) onto the blastodisc home on T 2.0, inOutCubic over 6 frames, on twos
      const kSet = tw < B_BUILD ? 0 : E.inOutCubic(clamp((tw - B_BUILD) / (6 * FR) + 2 / 6));
      const sparkY = lerp(700, HOME[1], kSet);
      if (t >= B_BUILD && t < B_BUILD + 6 * FR) {
        const fade = 1 - (t - B_BUILD) / (6 * FR);
        if (fade > 0) {
          const tr = new Path2D();
          tr.moveTo(HOME[0], 700);
          tr.lineTo(HOME[0], sparkY);
          ctx.save();
          const gr = ctx.createLinearGradient(HOME[0], 700, HOME[0], sparkY + 1);
          gr.addColorStop(0, L.rgba(mag, 0.45));
      ...
      ring(HOME[0], HOME[1], B_DIV2, 4, 20, 70, 3);
      ring(HOME[0], HOME[1], B_DIV4, 4, 30, 90, 3);
      if (t >= B_DIV8 && t < B_DIV8 + 4 * FR) for (const n of nuclei) ring(n[0], n[1], B_DIV8, 4, 6, 18, 2);
      // the pip signal: 8 magenta lines radiate from the pip point for 4 frames
      if (t >= B_EMB && t < B_EMB + 4 * FR) {
        const u = (t - B_EMB) / (4 * FR);
        const hr = L.rng(sd('pip'));
        const hp = new Path2D();
        for (let i = 0; i < 8; i++) {
          const a = (i / 8) * TAU + hr.range(-8, 8) * DEG;
          const len = hr.range(34, 70);
          const r0 = 12 + 10 * u;
          const r1 = r0 + len * E.outExpo(clamp((u + 0.25) * 2.5));
          hp.moveTo(PIP[0] + Math.cos(a) * r0, PIP[1] + Math.sin(a) * r0);
          hp.lineTo(PIP[0] + Math.cos(a) * r1, PIP[1] + Math.sin(a) * r1);
        }
        ctx.save();
        ctx.strokeStyle = mag;
        ctx.lineWidth = 3;
      ...
      // the spark itself: 8 spikes flaring 0 to 40 px over 3 frames, twinkling on twos, then the nucleus
      if (t < B_BIRTH) {
        const lf = Math.floor(t * 24 + 1e-6);
        const flare = [0.34, 0.68, 1][lf] || 1;
        const tk = drawing(0) % 2;
        const r = (40 / 3.4) * flare * (tk ? 0.86 : 1);
        const settle = hit(B_BUILD, 6);
        L.glowDot(ctx, HOME[0], sparkY, lerp(r, 9, settle), {
          rays: 8,
          rayLen: lerp(3.4, 2, settle),
          rot: tk * 22.5 * DEG,
          color: P.glow,
          glow: 5,
          seed: 70,
      ...
      // ---- network insets: blotch pigment (left) and the pore net (right) -----------------
      ...
      nodeGlyph(190, 1430, hit(B_LAT + 0.25, 3, E.outBack), bl.x, bl.y, bl.rad + 10, sd('node-l'), () => {
      ...
      nodeGlyph(868, 1430, hit(B_LAT + 0.375, 3, E.outBack), px, py, 22, sd('node-r'), () => {
        ctx.rotate(-0.1);
        L.stipple(ctx, null, {
      ...
      // ---- glyphs: division counter plate (top left) and the cycle ring (top right) --------
      if (aBg > 0) {
        ctx.save();
        ctx.globalAlpha = aBg;
        const plate = new Path2D();
        ...
        for (let c = 0; c < 4; c++) {
          ...
          for (let j = 0; j < n; j++) {
            ...
            if (k > 0) {
              ctx.globalAlpha = aBg * Math.min(1, k) * 0.9;
              ctx.fillStyle = white;
        ...
        ctx.stroke();
        ctx.globalAlpha = aBg;
        // the egg arc lights on T 2.0 and fills to 100 percent by T 4.0
        cycleRing(ctx, L, 0, clamp((t - B_BUILD) / (dur - B_BUILD)), bi);
        ctx.restore();
      }
    },
  });
