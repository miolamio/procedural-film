...
(function () {
  ...
  const MIX = {
    ...
    ride: [[0, -1], [5.5, -1.5], [7, 0], [10, 0], [11.7, -1]],
  };
  ...
  // ---------------------------------------------------------------- the score
  // THE SCORE — "North Is Where the Sky Stands Still". 120 bpm, 12 s, 6 bars. Night pad in A minor,
  // glass for stars, bells for the two poles (A for Polaris, a tritone off for Betelgeuse), paper
  // thumps and ink taps for the funnel, wing whooshes for the flight. Every cue of
  // FILM.TIMELINE.cues is implemented below at its own time (comments give the cue).
  const CH = {
    home: ['A2', 'E3', 'B3', 'C4', 'E4'], // A minor add9
    fmaj7: ['F2', 'C3', 'E3', 'A3', 'C4'],
    low: ['A1', 'E2'],
    dmin: ['D3', 'F3', 'A3', 'C4', 'E4'], // D minor add9
  };

  function score(E, I) {
    const { kick, tock, glass, fmBell, kalimba, pad, sub, nz, revSwell } = I;
    const R = (t) => Math.round(t * 1000) / 1000;
    const padO = { att: 0.4, rel: 0.6, cut0: 700, cut1: 1500, hall: 0.35, detune: 6 };

    // ---- harmony bed
    pad(0, 3, CH.home, 0.26, padO); // T 0 swell: night pad opens
    sub(0, 3, 'A1', 0.22, { att: 0.3, rel: 0.3 });
    pad(3, 5.5, CH.fmaj7, 0.26, Object.assign({}, padO, { att: 0.25 })); // T 3 swell: pad to F maj7
    sub(3, 5.5, 'F1', 0.22, { att: 0.2, rel: 0.2 });
    pad(5.5, 7, CH.low, 0.2, { att: 0.05, rel: 0.3, cut0: 400, cut1: 500, sine: true, hall: 0.3 }); // T 5.5: pad drops to A2
    pad(7, 10, CH.dmin, 0.24, Object.assign({}, padO, { att: 0.2, cut0: 600 })); // T 7: pad in D minor
    sub(7, 8.5, 'A1', 0.3, { att: 0.02, rel: 0.2 }); // planetarium hum, 55 Hz ...
    pad(10, 12, CH.home, 0.26, Object.assign({}, padO, { att: 0.15, rel: 0.4 })); // T 10: resolves home
    sub(10, 12, 'A1', 0.22, { att: 0.1, rel: 0.2 });

    // ---- a soft heart on the downbeats of the learning act and the flight
    for (const t of [0, 2, 4, 10]) kick(t, 0.32, 'felt');

    // ---- act 1: stars
    glass(0, hz('A5'), 0.5, { dec: 2.4, hall: 0.5, delay: 0.15 }); // T 0: ping for Polaris
    kalimba(0, hz('E5'), 0.28, { hall: 0.3, pan: -0.25 });
    [['G5', 0.5], ['A5', 1], ['C6', 1.5]].forEach(([n, t], i) => kalimba(t, hz(n), 0.3, { hall: 0.3, delay: 0.12, pan: -0.25 + 0.2 * i })); // Dipper edges
    // T 2: the turn — a low swell and 16th clicks that breathe with the turn
    revSwell(2, 0.5, 0.12, { fTop: 1200, hall: 0.4 });
    tock(2, 0.5, 1800, { pan: 0 });
    for (let k = 1; k < 32; k++) {
      const t = R(2 + k * 0.0625);
      const u = Math.sin((Math.PI * k) / 32);
      tock(t, 0.08 + 0.22 * u, 1800 + 600 * u, { pan: Math.sin(k * 0.7) * 0.4, dec: 0.03 });
    }
    kalimba(3, hz('C5'), 0.26, { hall: 0.3 });
    // T 4: bell on Polaris; T 4.5: the pointer figure; T 5: the pulse
    fmBell(4, hz('A5'), 0.45, { dec: 2.6, hall: 0.45 });
    fmBell(4, hz('E6'), 0.25, { dec: 2.2, hall: 0.45, pan: 0.2 });
    [['E5', 4.5], ['A5', 4.75], ['E6', 5]].forEach(([n, t]) => glass(t, hz(n), 0.4, { dec: 1.4, hall: 0.35, delay: 0.1 }));
    kick(5, 0.5, 'heart');

    // ---- act 2: the funnel, the false pole, the funnel
    const paper = (t) => {
      kick(t, 0.55, 'thud');
      tock(t, 0.4, 700, { dec: 0.03 }); // the paper's crack on the cut
      nz(t, 0.25, { type: 'lowpass', q: 0.8, f: [[0, 900], [0.2, 180, 'exp']], amp: [[0, 0], [0.004, 0.5], [0.22, FLOOR, 'exp']], key: 'paper', room: 0.3 });
    };
    const taps = (t, seed) => {
      tock(t, 0.4, 2400, { pan: -0.2 + 0.1 * seed, dec: 0.02 });
      tock(R(t + 0.0625), 0.25, 2700, { pan: 0.2, dec: 0.02 });
    };
    paper(5.5); // T 5.5: the plate inverts
    taps(6, 0); taps(6.5, 1);
    tock(7, 0.45, 900, { dec: 0.05 }); // T 7: projector click
    fmBell(7.5, hz('D#5'), 0.45, { dec: 2.4, hall: 0.45 }); // T 7.5: bell on Betelgeuse
    fmBell(7.5, hz('A5'), 0.25, { dec: 2, hall: 0.45, pan: -0.2 });
    paper(8.5);
    taps(9, 2); taps(9.5, 3);

    // ---- act 3: flight
    for (const t of [10, 10.5, 11, 11.5]) {
      nz(t, 0.36, { type: 'bandpass', q: 1.2, f: [[0, 400], [0.3, 2000, 'exp']], amp: [[0, 0], [0.01, 0.1], [0.12, 0.3, 'lin'], [0.36, FLOOR, 'exp']], key: 'wing', stereo: true, hall: 0.2 });
    }
    glass(11.5, hz('A5'), 0.5, { dec: 3, hall: 0.6, delay: 0.2 }); // T 11.5: Polaris again, into the loop
  }
