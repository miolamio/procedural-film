// 04 funnel-autumn — Footprints away from the pole. T 5.5 to 7.
// The Emlen funnel from above (G3) on plate B: the bird hops to the lower wall on T 6.0 and 6.5 and
// its inked three-toed prints gather in a 70 deg sector away from Polaris (the accent marker, top).
// This file owns the FUNNEL block: 06 copies it verbatim.
...
(function () {
  ...
  FILM.scene({
    ...
    draw(ctx, tIn, info) {
      ...
      FUNNEL.draw(ctx, T, {
        frame: info.frame,
        marker: G3.pt('poleTrue'),
        cluster: G3.pt('clusterTrue'),
        seedId: ID, // prints seeded lib.hash(ID, i)
        hops: [6.0, 6.5], // global T, the two hop beats
        bracketT: 6.75,
      });
    },
  });
