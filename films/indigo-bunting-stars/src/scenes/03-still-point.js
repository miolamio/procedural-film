// 03 still-point — The star that stays. T 4 to 5.5.
// Push-in 1 to 1.45 about Polaris. The turned sky (phi 40 to 44 deg) with faint trails; the
// instrument lands on Polaris (T 4), the pointer line Merak–Dubhe–Polaris draws (T 4.5), the dotted
// ring pulses (T 5). The bird slides toward the lower-left edge as the camera closes in.
// Layers, back to front: 1 plate (screen-fixed), 2 dust, under the camera: 3 band + stars, 4 guides,
// 5 trails at 30%, 6 edges, 7 named stars + Polaris, 8 pointer line, 9 instrument, 10 sight line, twig, bunting.
// WORLD block copied verbatim from 01-first-summer.js.
(function () {
  'use strict';
  const FILM = window.FILM;
  const ID = 'still-point';
  ...
  const P = W.P;
  const G1 = LIB.geo('G1');
  const T_RING = 4.0; // beat 1 of bar 3
  const T_POINT = 4.5; // beat 2
  const T_PULSE = 5.0; // beat 3

  // the instrument on the pole: ring r 6, cross ±16, dotted ring r 40, dashed ellipse r 150 with
  // ticks turning with the sky, two long diagonals (theme 5, "Instrument")
  function drawInstrument(ctx, T, a, z) {
    const [x, y] = W.POLE;
    const on = LIB.hit(T, T_RING, 3, 'outBack');
    if (!(on > 0)) return;
    const pulse = T >= T_PULSE ? Math.sin(Math.PI * W.cl((T - T_PULSE) / 0.5)) : 0;
    const lw = 1 / z; // hold screen line weights under the push-in
    ctx.save();
    ctx.strokeStyle = P.lineFaint;
    ctx.globalAlpha = 0.18;
    ctx.lineWidth = lw;
    for (const d of [0.62, 2.2]) {
      ctx.beginPath();
      ctx.moveTo(x - Math.cos(d) * 2400, y - Math.sin(d) * 2400);
      ctx.lineTo(x + Math.cos(d) * 2400, y + Math.sin(d) * 2400);
      ctx.stroke();
    }
    ctx.strokeStyle = P.lineSoft;
    ctx.globalAlpha = 0.55 * Math.min(1, on);
    ctx.lineWidth = 1.2 * lw;
    ctx.setLineDash([10 * lw, 8 * lw]);
    ctx.beginPath(); ctx.arc(x, y, 150 * on, 0, W.TAU); ctx.stroke();
    ctx.setLineDash([]);
    ctx.beginPath();
    for (let i = 0; i < 36; i++) {
      const q = i * 10 * W.DEG - a, L = (i % 6 === 0 ? 16 : 8) * lw;
      ctx.moveTo(x + Math.cos(q) * 150 * on, y + Math.sin(q) * 150 * on);
      ctx.lineTo(x + Math.cos(q) * (150 * on - L), y + Math.sin(q) * (150 * on - L));
    }
    ctx.stroke();
    // dotted ring r 40, pulsing once at T 5
    const r40 = 40 * on * (1 + 0.25 * pulse);
    ctx.fillStyle = P.line;
    ctx.globalAlpha = 0.7;
    for (let i = 0; i < 28; i++) {
      const q = (i / 28) * W.TAU - a;
      ctx.beginPath(); ctx.arc(x + Math.cos(q) * r40, y + Math.sin(q) * r40, 1.3 * lw, 0, W.TAU); ctx.fill();
    }
    // ring r 6 and cross ±16
    ctx.strokeStyle = P.line;
    ctx.globalAlpha = 0.9;
    ctx.lineWidth = 1.4 * lw;
    ctx.beginPath(); ctx.arc(x, y, 6 * on + 3, 0, W.TAU); ctx.stroke();
    ctx.beginPath();
    ctx.moveTo(x - 16 * on, y); ctx.lineTo(x - 10, y);
    ctx.moveTo(x + 10, y); ctx.lineTo(x + 16 * on, y);
    ctx.moveTo(x, y - 16 * on); ctx.lineTo(x, y - 10);
    ctx.moveTo(x, y + 10); ctx.lineTo(x, y + 16 * on);
    ctx.stroke();
    ctx.restore();
    W.drawPolaris(ctx, x, y, 1, pulse);
  }

  // the pointer line: Merak through Dubhe to Polaris, dashed, drawing on from T 4.5 over one beat
  function drawPointer(ctx, T, a, z) {
    const u = LIB.hit(T, T_POINT, 12, 'outExpo');
    if (!(u > 0)) return;
    const M = W.turn(G1.pt('merak'), a), D = W.turn(G1.pt('dubhe'), a), Q = W.POLE;
    const L1 = Math.hypot(D[0] - M[0], D[1] - M[1]), L2 = Math.hypot(Q[0] - D[0], Q[1] - D[1]) - 14;
    let rem = (L1 + L2) * u;
    const lw = 1 / z;
    ctx.save();
    ctx.strokeStyle = P.lineSoft;
    ...
    seg(M, D, rem);
    rem -= L1;
    if (rem > 0) seg(D, Q, rem);
    ctx.stroke();
    ctx.setLineDash([]);
    // the five gaps: brackets marking Merak–Dubhe and its five repeats along the line
    ctx.strokeStyle = P.lineSoft;
    ctx.lineWidth = 1.4 * lw;
    ctx.globalAlpha = 0.75 * W.cl((u - 0.6) / 0.4);
    const ux = (D[0] - M[0]) / L1, uy = (D[1] - M[1]) / L1, nx = -uy, ny = ux;
    ctx.beginPath();
    for (let i = 0; i <= 6; i++) {
      const px = M[0] + ux * L1 * i, py = M[1] + uy * L1 * i;
      const h = (i === 0 || i === 1 ? 14 : 8) * lw;
      ctx.moveTo(px + nx * h, py + ny * h); ctx.lineTo(px - nx * h, py - ny * h);
    }
    ctx.stroke();
    ctx.restore();
  }
  ...
  FILM.scene({
    ...
    draw(ctx, tIn, info) {
      ...
      const a = W.phi(T);
      const z = 1 + 0.45 * LIB.ease.inOutSine(t / info.dur);
      const [px, py] = W.POLE;
      W.drawPlate(ctx);
      W.drawDust(ctx, T);
      // the camera holds Polaris on its screen place while it closes in
      const cam = { x: px - (px - 960) / z, y: py - (py - 540) / z, zoom: z };
      LIB.camera(ctx, cam, () => {
        W.drawBand(ctx, a);
        W.drawField(ctx, T, a);
        W.drawGuides(ctx, a);
        W.drawTrails(ctx, a, 0.3);
        W.drawEdges(ctx, W.CAS, a, P.lineSoft, 0.28);
        W.drawEdges(ctx, W.BIG, a, P.line, 0.6);
        W.drawEdges(ctx, W.LITTLE, a, P.lineSoft, 0.45);
        W.drawNamed(ctx, a, 1, 0);
        drawPointer(ctx, T, a, z);
        drawInstrument(ctx, T, a, z);
        if (T < T_RING) W.drawPolaris(ctx, px, py);
