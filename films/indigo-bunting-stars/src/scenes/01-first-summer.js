// 01 first-summer — The first summer. T 0 to 2.
// The young bunting, a constellation, perched lower left and looking up at Polaris; the sky of G1
// at phi 0; the Dippers' edges draw on one per 8th; a dashed sight line reaches for the pole.
// Layers, back to front: 1 plate, 2 dust, 3 background stars, 4 constellation edges, 5 named stars,
// 6 Polaris (accent), 7 sight line, 8 twig, 9 bunting.
// This file owns the WORLD block: 02, 03 and 07 copy it verbatim.
(function () {
  'use strict';
  const FILM = window.FILM;
  const ID = 'first-summer';
  ...
  FILM.scene({
    ...
    draw(ctx, tIn, info) {
      ...
      W.drawField(ctx, T, a);
      W.drawGuides(ctx, a);
      // 4 edges: the Big Dipper one per 8th from T 0, the Little Dipper from T 0.25, Cassiopeia faint and whole
      const rev = (t0) => (i) => LIB.hit(T, t0 + i * 0.25, 6, 'outExpo');
      W.drawEdges(ctx, W.CAS, a, W.P.lineSoft, 0.28);
      W.drawEdges(ctx, W.BIG, a, W.P.line, 0.55, rev(0));
      W.drawEdges(ctx, W.LITTLE, a, W.P.lineSoft, 0.4, rev(0.25));
      // 5, 6
      W.drawNamed(ctx, a);
      // 7 sight line from T 1.0 over two beats
      const breath = 1 + 0.03 * Math.sin((T / 2) * W.TAU);
      const tilt = -3 * W.DEG * LIB.hit(T, 1.0, 12, 'inOutSine');
      const bird = { T, frame: info.frame, tilt, breath, perchY: W.perchY };
      W.drawSight(ctx, W.birdEye(bird), W.POLE, LIB.ease.outExpo(W.cl((T - 1) / 1)));
      // 8, 9
      W.drawTwig(ctx, T);
