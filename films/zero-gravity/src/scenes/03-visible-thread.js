// 03 visible-thread — The thread shows (scenario S05_03). T 42.105 to 66.105 (bars 24–37).
// The amber thread draws from A's rim to B's over two beats, once, then bends on the loop; the orbit gives way to two diverging trails.
// Layers: WORLD.drawAll (the thread's reveal passed in) + the distance bracket beside the thread.
(function () {
  'use strict';
  const FILM = window.FILM;
  const ID = 'visible-thread';
  ...
  FILM.scene({
    ...
    draw(ctx, tIn, info) {
      ...
      const REVEAL = 2 * WORLD.BEAT;
      const reveal = L.ease.inOutSine(WORLD.cl(t / REVEAL + 1 / 21));
      WORLD.drawAll(ctx, T, {
        reveal,
        top(c) {
          // overlay: a distance bracket parallel to the chord, on the side away from the bow
          const k = WORLD.ss(REVEAL, REVEAL + WORLD.BEAT, t);
          if (k <= 0) return;
          const th = WORLD.thread(T).pts;
          const p0 = th[0], p1 = th[th.length - 1], mid = th[th.length >> 1];
          const dx = p1[0] - p0[0], dy = p1[1] - p0[1], l = Math.hypot(dx, dy) || 1;
          let nx = -dy / l, ny = dx / l;
          const cx = (p0[0] + p1[0]) / 2, cy = (p0[1] + p1[1]) / 2;
          if ((mid[0] - cx) * nx + (mid[1] - cy) * ny > 0) { nx = -nx; ny = -ny; }
          const off = 44;
          const a = [p0[0] + nx * off, p0[1] + ny * off], b = [p1[0] + nx * off, p1[1] + ny * off];
          c.lineWidth = 1.4;
          c.strokeStyle = L.rgba(P.lineFaint, 0.55 * k);
          c.beginPath();
          c.moveTo(a[0], a[1]); c.lineTo(b[0], b[1]);
          for (const q of [a, b]) { c.moveTo(q[0] - nx * 7, q[1] - ny * 7); c.lineTo(q[0] + nx * 7, q[1] + ny * 7); }
          c.stroke();
          // small inner ticks every 60 px along the bracket
          c.strokeStyle = L.rgba(P.lineFaint, 0.35 * k);
          c.beginPath();
          for (let d = 60; d < l - 20; d += 60) {
            const x = a[0] + (dx / l) * d, y = a[1] + (dy / l) * d;
            c.moveTo(x, y); c.lineTo(x - nx * 5, y - ny * 5);
          }
          c.stroke();
        },
      });
