// 08 last-string — One string (scenario S05_08). T 165.534 to 172.042 (bar 96 to the end of the song).
// Only the thread is left, nearly horizontal; a residual tremble repeats every bar (8 pulses).
// Layers: WORLD.drawAll (the bodies left in S05_07; the thread is the string).
(function () {
  'use strict';
  ...
  FILM.scene({
    ...
    draw(ctx, tIn, info) {
      ...
      WORLD.drawAll(ctx, T, { threadWidth: 2.4 + 0.2 * WORLD.cl(t / 1.5) });
    },
  });
})();
