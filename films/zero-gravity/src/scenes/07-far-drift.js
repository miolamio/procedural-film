// 07 far-drift — Far apart (scenario S05_07). T 144.962 to 165.534 (bars 84–95).
// A left, B right, a wide void between; slow independent drift on the loop; from bar 92 both leave the frame, once.
// Layers: WORLD.drawAll + a faint dashed gap rule below the pair.
(function () {
  'use strict';
    ...
    draw(ctx, tIn, info) {
      ...
      WORLD.drawAll(ctx, T, {
        afterOrbit(c) {
          ...
          const y = Math.max(A.y, B.y) + 190;
          const x0 = A.x + 20, x1 = B.x - 20;
          const k = L.hit(t, 0, 12, 'outExpo') * WORLD.layer.exitFade(T); // leaves with the bodies
          if (k <= 0.001 || x1 <= x0) return;
          c.lineWidth = 1.4;
          ...
          c.moveTo(x1, y - 7); c.lineTo(x1, y + 7);
          c.stroke();
        },
      });
    },
