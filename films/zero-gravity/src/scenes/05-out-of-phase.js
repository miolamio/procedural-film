// 05 out-of-phase — Out of phase (scenario S05_05). T 93.534 to 117.534 (bars 54–67), the hinge.
// The breathing slides into opposite phase: one shell at full expansion while the other contracts.
// The turning dies away, B lags, B's field flows inward, the centre breaks into fragments, gone by bar 62.
// Layers: WORLD.drawAll + a phase mark (short arc) outside each shell, lit at that shell's widest.
(function () {
  'use strict';
  const FILM = window.FILM;
  const ID = 'out-of-phase';
  ...
  FILM.scene({
    ...
    draw(ctx, tIn, info) {
      ...
      WORLD.drawAll(ctx, T, {
        top(c) {
          const A = WORLD.body('A', T), B = WORLD.body('B', T);
          for (const [w, me, other] of [['A', A, B], ['B', B, A]]) {
            const br = WORLD.breath(w, T);
            const full = WORLD.cl((br - 0.88) / 0.24);
            const away = Math.atan2(me.y - other.y, me.x - other.x);
            const r = WORLD.shellRadius(w, T) * 1.3 + 22;
            c.lineWidth = 2;
            c.lineCap = 'round';
            c.strokeStyle = L.rgba(P.line, 0.12 + 0.7 * Math.pow(full, 3));
            c.beginPath(); c.ellipse(me.x, me.y, r, r * 0.92, 0, away - 0.45, away + 0.45); c.stroke();
          }
        },
      });
    },
