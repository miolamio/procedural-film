// 02 breathing-shells — Shells breathe together (scenario S05_02). T 14.676 to 42.105 (bars 8–23).
// On the downbeat each body grows its own two-line shell, once; the shells breathe one breath per 16-pulse loop, in sync.
// Layers: WORLD.drawAll (plate, dust, guides, orbit, centre, shells, bodies) + 12 breath ticks on each inner shell.
(function () {
  'use strict';
  const FILM = window.FILM;
  const ID = 'breathing-shells';
  ...
  FILM.scene({
    ...
    draw(ctx, tIn, info) {
      ...
      WORLD.drawAll(ctx, T, {
        top(c) {
          // overlay: breath ticks at the inner shell's extent, brightest at full inhale
          const k = WORLD.layer.shells(T);
          if (k <= 0) return;
          for (const w of ['A', 'B']) {
            const b = WORLD.body(w, T);
            const br = WORLD.breath(w, T);
            const grow = WORLD.layer.shellGrow(T);
            const pts = WORLD.contour(w, b, WORLD.lerp(1, 1.7, grow) * br, T, 0, 7);
            const full = WORLD.cl((br - 1) / 0.12 * 0.5 + 0.5);
            c.lineWidth = 1.3;
            c.strokeStyle = L.rgba(P.line, (0.25 + 0.55 * full * full) * k);
            c.beginPath();
            for (let i = 0; i < pts.length; i += 8) {
              const p = pts[i];
              const dx = p[0] - b.x, dy = p[1] - b.y, l = Math.hypot(dx, dy) || 1;
              c.moveTo(p[0] + (dx / l) * 5, p[1] + (dy / l) * 5);
              c.lineTo(p[0] + (dx / l) * 15, p[1] + (dy / l) * 15);
            }
            c.stroke();
          }
        },
