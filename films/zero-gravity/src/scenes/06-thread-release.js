// 06 thread-release — The thread lets go (scenario S05_06). T 117.534 to 144.962 (bars 68–83).
// Bars 68–69: tension (tremble, straightening). Bar 70 downbeat T 120.962: the thread breaks at B, once.
// Layers: WORLD.drawAll + the break ring and sparks at B's rim (WORLD.drawBreak).
(function () {
  'use strict';
  const FILM = window.FILM;
  const ID = 'thread-release';
  ...
  FILM.scene({
    ...
    draw(ctx, tIn, info) {
      ...
      WORLD.drawAll(ctx, T, { beforeThread(c) { WORLD.drawBreak(c, T); } });
    },
  });
})();
