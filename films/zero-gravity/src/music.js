...
(function () {
  ...
  const MIX = {
    ...
    // Section fader rides in dB at global times, pre-compressor: a hushed orbit, a slow lift to the
    // riser, full level for the snap, then the void and the lone string back down near the opening
    // level so the loop seam does not jump.
    ride: (() => {
      const B = (n) => 0.105 + 6 / 7 + n * (12 / 7);
      const SNAP = B(70);
      return [
        [0, -6.5], [B(24), -6.5], [B(54), -5.5], [B(68), -5.5], [SNAP - 0.02, -4.5], [SNAP, -2], [B(72), -3],
        [B(84), -4], [172.042449, -5],
      ];
    })(),
  };
  ...
  // ---------------------------------------------------------------- the score
  // "Zero Gravity and No Attraction": a draft score for viewing, the whole song (2:52.042) on the
  // recording's grid: 140 bpm, first beat 0.105 s, D dorian, suspended harmony with no cadence.
  // On stage the film runs under the live song; this score only has to sit on the same grid.
  const CH = {
    Dm9: ['D3', 'A3', 'C4', 'E4', 'F4'],
    Bbmaj7: ['Bb2', 'F3', 'A3', 'D4', 'E4'], // with the #11 (E) left hanging
    Gm9: ['G2', 'D3', 'Bb3', 'F4', 'A4'],
  ...
  function score(E, I) {
    const { kick, tock, kalimba, marimba, glass, fmBell, gong, sub, subDrop, pluck, nz, revSwell, glide } = I;
    const BEAT = 3 / 7;
    const BAR = 12 / 7;
    const E8 = BEAT / 2;
    const DUR = (FILM.TIMELINE && FILM.TIMELINE.duration) || 172.042449;
    // The song's grid (measured from the recording): first beat at 0.105 s, bar 0 two beats later.
    const T_BEAT0 = 0.105;
    const T_BAR0 = T_BEAT0 + 2 * BEAT;
    const beat = (i) => T_BEAT0 + i * BEAT; // beat i of the song
    const b = (k) => T_BAR0 + k * BAR; // downbeat of bar k
    const SNAP = b(70);
    const LEAVE = b(92);

    // Filter breath: exponential moves between absolute [t, hz] keys. pts(t0, tEnd) returns the
    // envelope for a voice starting at t0 (the value at t0, then every key inside the voice).
    const curve = (keys) => {
      const at = (t) => {
        ...
        for (let i = 1; i < keys.length; i++) {
          if (t <= keys[i][0]) {
            ...
            const [tb, fb] = keys[i];
            return fa * Math.pow(fb / fa, (t - ta) / (tb - ta));
          }
        }
      ...
      return (t0, tEnd) => {
        const out = [[0, at(t0)]];
        for (const [t, f] of keys) if (t > t0 + 1e-6 && t < tEnd - 1e-6) out.push([t - t0, f, 'exp']);
        out.push([tEnd - t0, at(tEnd), 'exp']);
        return out;
      };
    };
    // One breath per loop (two bars, like the shells): the cutoff peaks on each even downbeat and
    // sinks to a trough 55% into the loop. phase shifts the peaks by that fraction of a loop.
    const LOOP = 2 * BAR;
    const breath = (k0, k1, peak, trough, phase, closed) => {
      const keys = [[0, closed]];
      const p0 = b(k0) + phase * LOOP;
      keys.push([p0 - 0.45 * LOOP, closed]);
      for (let k = k0; k < k1; k += 2) {
        const p = b(k) + phase * LOOP;
        keys.push([p, peak]);
        keys.push([p + 0.55 * LOOP, trough]);
      }
      keys.push([b(k1) + phase * LOOP, peak]);
      return curve(keys);
    };
    const openA = breath(8, 70, 2300, 820, 0, 620);
    const openB = breath(54, 70, 2100, 780, 0.5, 780);

    // Sustained pad voice: warm saws (or sines) per note, one lowpass driven by a breath curve.
    // o: att, rel, cut (a curve), cents (whole-layer detune), spread (per-note side detune), width, pan, sine
    const drift = (t0, t1, notes, vel, o) => {
      const rel = o.rel;
      const len = t1 - t0 + rel;
      const V = E.voice(t0, len, true);
      if (!V) return;
      const lp = E.filt('lowpass', 800, o.q || 0.7);
      V.env(lp.frequency, o.cut(t0, t0 + len));
      const g = E.gain(0);
      const pts = [[0, 0], [o.att, vel]];
      if (t1 - t0 > o.att) pts.push([t1 - t0, vel * (o.sus === undefined ? 1 : o.sus)]);
      pts.push([len, 0]);
      V.env(g.gain, pts);
      lp.connect(g);
      E.out(g, o.bus || 'pad', o);
      const sides = o.sine ? [0] : [-1, 1];
      const per = 1 / Math.sqrt(notes.length * sides.length);
      notes.forEach((nm, i) => {
        for (const side of sides) {
          const s = E.osc(o.sine ? 'sine' : E.warmSaw, hz(nm));
          s.detune.value = (o.cents || 0) + side * (o.spread === undefined ? 6 : o.spread) + (i % 2 ? 1.5 : -1.5);
          const sg = E.gain(per * (o.sine ? 1.3 : 1));
          const p = E.panner(Math.max(-1, Math.min(1, (o.pan || 0) + side * (o.width === undefined ? 0.5 : o.width))));
          s.connect(sg);
          sg.connect(p);
          p.connect(lp);
          V.osc(s);
    ...
    // Bowed voice: two warm saws through a resonant lowpass, pitch, cutoff, vibrato and amplitude
    // all as envelopes. o: { f (pts, Hz), cut (pts), vib (pts, cents), amp (pts), q, bus, sends }
    const bowed = (t0, len, o) => {
      const V = E.voice(t0, len, true);
      if (!V) return;
      const lp = E.filt('lowpass', o.cut[0][1], o.q || 1.4);
      V.env(lp.frequency, o.cut);
      const g = E.gain(0);
      V.env(g.gain, o.amp);
      const lfo = E.osc('sine', o.rate || 5.2);
      const vg = E.gain(0);
      V.env(vg.gain, o.vib);
      lfo.connect(vg);
      for (const [d, a] of [[-4, 0.5], [4, 0.5], [0, 0.18]]) {
        const s = E.osc(d ? E.warmSaw : 'sine', o.f[0][1] * (d ? 1 : 0.5));
        V.env(s.frequency, d ? o.f : o.f.map(([t, f, sh]) => [t, f * 0.5, sh]));
        s.detune.value = d;
        vg.connect(s.detune);
        const sg = E.gain(a);
        s.connect(sg);
      ...
      lp.connect(g);
      E.out(g, o.bus || 'lead', o);
      V.osc(lfo);
    };

    // =========================================================== S05_01: the shared orbit
    // T 0: the low D drone and the air fade in over one bar; both hold until the snap cuts them.
    sub(0, SNAP - 0.012, 'D2', 0.34, { att: BAR, rel: 0.012, sus: 1.05 });
    nz(0, SNAP, {
      type: 'bandpass',
      q: 0.5,
      f: [[0, 3800], [b(8), 4200, 'exp'], [b(68), 5200, 'exp'], [SNAP - 0.012, 6000, 'exp']],
      type2: 'lowpass',
      f2: 7000,
      amp: [[0, 0], [BAR, 0.035], [b(68), 0.045], [SNAP - 0.012, 0.05], [SNAP, 0]],
      stereo: true,
      sustain: true,
      bus: 'amb',
      key: 'air',
    });
    // The felt pulse on every beat of the song from its first beat (the two pickup beats too), low
    // in the mix: the orbit's heartbeat. Out at the hinge.
    for (let i = 0; beat(i) < b(54) - 1e-6; i++) kick(beat(i), (i - 2) % 4 ? 0.2 : 0.26, 'felt');

    // The pad: closed and airy through S05_01, then one filter breath per loop (2 bars) peaking on
    // the even downbeats. Chords change every loop: Dm9, Bbmaj7, Gm9, A7sus4, around.
    const XF = 0.3;
    const CYCLE = ['Dm9', 'Bbmaj7', 'Gm9', 'A7sus4'];
    const padPlan = [[-1, 8, 'Dm9']];
    for (let k = 8, i = 0; k < 54; k += 2, i++) padPlan.push([k, Math.min(k + 2, 54), CYCLE[i % 4]]);
    // T b(8): the shells grow on the downbeat: a soft chime as the pad starts to breathe.
    glass(b(8), hz('D6'), 0.12, { att: 0.001, dec: 1.8, hall: 0.3, delay: 0.12, pan: -0.2 });
    glass(b(8), hz('A5'), 0.1, { att: 0.001, dec: 1.8, hall: 0.3, delay: 0.12, pan: 0.2 });
    padPlan.forEach(([k0, k1, ch], i) => {
      const t0 = i ? b(k0) - XF / 2 : 0;
      drift(t0, b(k1) - XF / 2, CH[ch], 0.34, { att: i ? XF : BAR, rel: XF, cut: openA, hall: 0.25, cave: 0.08 });
    });

    // =========================================================== S05_03: the thread
    // T b(24): the thread appears, a glassy pluck on A4 with a long tail; the harmonic two beats on.
    glass(b(24), hz('A4'), 0.25, { att: 0.001, dec: 4.5, hall: 0.3, cave: 0.35, delay: 0.1 });
    pluck(b(24), 'A4', 0.16, { dec: 1.4, bright: 14, hall: 0.2, cave: 0.3 });
    glass(b(24) + 2 * BEAT, hz('A6'), 0.12, { att: 0.001, dec: 2.2, hall: 0.3, cave: 0.3, delay: 0.16, pan: 0.2 });
    // The thread's bend, once per loop: a faint pluck on each even downbeat after the reveal.
    for (let k = 26; k < 38; k += 2) pluck(b(k), k % 4 ? 'E4' : 'A4', 0.06, { dec: 1.1, bright: 10, hall: 0.25, cave: 0.3, pan: 0.15 });

    // =========================================================== S05_04 and S05_05: the fields
    // T b(38): two soft arpeggio voices in 8ths, A on the beats (left), B on the off-beats (right),
    // a four-bar pattern. From the hinge B lags a 16th behind its off-beat and both thin out by b(62).
    const arp = [
      [['D4', 'F4', 'A4', 'E4'], ['A5', 'F5', 'D5', 'E5']],
      [['D4', 'E4', 'G4', 'E4'], ['G5', 'E5', 'D5', 'A4']],
      [['D4', 'F4', 'A4', 'E4'], ['C5', 'A4', 'F5', 'E5']],
      [['D4', 'F4', 'Bb4', 'A4'], ['Bb4', 'A5', 'F5', 'D5']],
    ];
    for (let k = 38; k < 62; k++) {
      const [left, right] = arp[(k - 38) % 4];
      const fade = k < 54 ? 1 : 1 - (k - 54) / 8;
      for (let j = 0; j < 4; j++) {
        const t = b(k) + j * BEAT;
        kalimba(t, hz(left[j]), (k === 38 && j === 0 ? 0.26 : 0.17) * fade, { dec: 1.3, pan: -0.55, hall: 0.2, delay: 0.08 });
        const lag = k >= 54 ? E8 / 2 : 0;
        marimba(t + E8 + lag, hz(right[j]), 0.1 * fade, { dec: 1.0, pan: 0.55, hall: 0.22, delay: 0.1 });
      }
    }

    // T b(54): the pad splits into two detuned layers that beat against each other, A breathing on
    // the even downbeats (left), B a bar later (right). Both cut dead with the snap.
    // The hinge itself: a soft two-note chime on the downbeat, one per side, a 16th apart.
    glass(b(54), hz('D5'), 0.14, { att: 0.001, dec: 2.4, hall: 0.3, cave: 0.3, pan: -0.4 });
    glass(b(54) + E8 / 2, hz('A5'), 0.1, { att: 0.001, dec: 2.4, hall: 0.3, cave: 0.3, pan: 0.4 });
    const split = [];
    for (let k = 54, i = 0; k < 70; k += 2, i++) split.push([k, k + 2, CYCLE[i % 4]]);
    split.forEach(([k0, k1, ch], i) => {
      const t0 = b(k0) - XF / 2;
      const last = i === split.length - 1;
      const t1 = last ? SNAP - 0.012 : b(k1) - XF / 2;
      const rel = last ? 0.012 : XF;
      drift(t0, t1, CH[ch], 0.3, { att: XF, rel, cut: openA, cents: -8, spread: 2, pan: -0.45, width: 0.25, hall: 0.25, cave: 0.1 });
      drift(t0, t1, CH[ch], 0.3, { att: XF, rel, cut: openB, cents: 8, spread: 2, pan: 0.45, width: 0.25, hall: 0.25, cave: 0.1 });
    });

    // =========================================================== S05_06: tension, then THE SNAP
    // T b(68): two bars of tension. The thread trembles (a tick), a bowed tone climbs A3 to A4 and a
    // noise swell rises under it; both cut dead on the snap.
    const RISE = SNAP - b(68);
    tock(b(68), 0.16, 2600, { bus: 'sfx', dec: 0.05, hall: 0.3, pan: 0.2 });
    bowed(b(68), RISE + 0.01, {
      f: [[0, hz('A3')], [RISE, hz('A4'), 'exp']],
      cut: [[0, 380], [RISE, 4200, 'exp']],
      vib: [[0, 4], [RISE, 22, 'lin']],
      rate: 6.2,
      amp: [[0, 0], [0.4, 0.03], [RISE - 0.003, 0.2, 'exp'], [RISE, 0]],
      q: 2.2,
      hall: 0.3,
    });
    revSwell(b(68), RISE, 0.26, { fTop: 5200, hall: 0.3 });

    // T b(70) = 120.962: the only big hit. A bright string pluck, a noise crack, a sub drop, a long
    // reverb, and the free end whipping back.
    E.duck(SNAP, 0.5);
    pluck(SNAP, 'D4', 0.15, { dec: 2.6, bright: 22, hall: 0.4, cave: 0.55 });
    pluck(SNAP, 'A4', 0.08, { dec: 2.0, bright: 22, hall: 0.4, cave: 0.5, pan: 0.25 });
    glass(SNAP, hz('D6'), 0.26, { att: 0.001, dec: 3.2, cave: 0.6, hall: 0.3, pan: 0.3 });
    nz(SNAP, 0.05, { type: 'highpass', q: 0.7, f: [[0, 3200]], amp: perc(0.32, 0.0003, 0.018), stereo: true, cave: 0.6, hall: 0.3, key: 'crack' });
    nz(SNAP, 0.12, { type: 'bandpass', q: 1.2, f: [[0, 2400], [0.1, 900, 'exp']], amp: perc(0.45, 0.0005, 0.05), pan: 0.3, cave: 0.5, key: 'crack-body' });
    kick(SNAP, 0.3, 'full');
    gong(SNAP, hz('D3'), 0.3, { dec: 3.8, hall: 0.3, cave: 0.5 });
    subDrop(SNAP, 110, 29, 1.8, 0.55);
    nz(SNAP, 3.4, { type: 'lowpass', q: 0.5, f: [[0, 5000], [3.4, 700, 'exp']], amp: [[0, 0], [0.01, 0.16], [0.6, 0.1, 'exp'], [3.4, FLOOR, 'exp']], stereo: true, cave: 0.7, bus: 'amb', key: 'snap-wash' });
    glide(SNAP + 0.03, 2400, 520, 0.55, 0.05, { hall: 0.4, cave: 0.4, pan: -0.3 });
    // The free end swings once per bar (8 pulses): a low, soft pluck on each downbeat, from A's side.
    for (let k = 72; k < 84; k++) pluck(b(k), k % 2 ? 'A3' : 'D3', 0.05, { dec: 1.6, bright: 6, hall: 0.3, cave: 0.35, pan: -0.35 });
    // The void opens under it: a high held fifth from bar 72 to bar 96.
    drift(b(72), b(96) - BAR, CH.fifth, 0.16, { att: 2 * BAR, rel: BAR, sine: true, cut: curve([[0, 3000]]), width: 0.4, hall: 0.35, cave: 0.3 });

    // =========================================================== S05_07: far apart, then gone
    // T b(84): one soft bell per bar, D6 and A5 in turn.
    for (let k = 84; k < 92; k++) fmBell(b(k), hz(k % 2 ? 'A5' : 'D6'), 0.13, { ratio: 2, index: 1.2, dec: 2.6, hall: 0.35, cave: 0.35, pan: k % 2 ? 0.25 : -0.25 });
    // T b(92): the bodies leave: a tick and a slow falling glide.
    tock(LEAVE, 0.12, 1800, { bus: 'sfx', dec: 0.06, hall: 0.35, pan: 0.3 });
    glide(LEAVE, 1400, 180, 3 * BAR, 0.08, { hall: 0.45, cave: 0.4, pan: 0.2 });

    // =========================================================== S05_08: one string
    // T b(96): a single sustained string tone on D3: a bow catch, then a long tone whose vibrato
    // decays to a residual tremble. It is still sounding at the last frame.
    const len = DUR - b(96) + 0.05;
    nz(b(96), 0.06, { type: 'bandpass', q: 1.1, f: [[0, 2600]], amp: perc(0.3, 0.0008, 0.03), hall: 0.3, key: 'bow' });
    bowed(b(96), len, {
      f: [[0, hz('D3')]],
      cut: [[0, 2600], [0.35, 1500, 'exp'], [len, 900, 'exp']],
      vib: [[0, 0], [0.3, 1], [1.1, 16, 'lin'], [len, 1.5, 'exp']],
      rate: 4.8,
      amp: [[0, 0], [0.004, 0.26], [0.3, 0.17, 'exp'], [len, 0.12, 'exp']],
      q: 1.1,
      hall: 0.35,
      cave: 0.2,
    });
