...
A shot may declare `transitionIn: { dur, kind }` in the timeline; `core` handles supported kinds (`cut` default, `fade`, `flash`, `iris`, `wipe`, `whip`, `inkwash`, `morph`, `crtoff`) by drawing both shots into offscreen layers.
...
- On the first frame of `whip`, `inkwash`, `morph` and `crtoff` the picture is the outgoing shot alone; once `dur` has elapsed it is the incoming shot alone. `fade`, `iris` and `wipe` are already partway across on their first frame.
