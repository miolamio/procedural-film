...
//   --carrier none | crt | vhs | film | xerox     the medium over the whole film: the timeline's carrier
...
const CARRIERS = ['none', 'crt', 'vhs', 'film', 'xerox'];
...
function main() {
  ...
  try {
    ...
    } else throw new Error("usage: node tools/theme.cjs list | apply <id> [--frame WxH] [--carrier none|crt|vhs|film|xerox] [--accent '#RRGGBB'] [--grain 0..1] | show | lookbook [--out path]");
  } catch (e) {
    C.die(e.message);
  }
}
