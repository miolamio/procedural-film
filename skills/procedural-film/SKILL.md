...
`node tools/check.cjs` is the gate: twelve checks (media scan, determinism, source scan, timeline, draw, frame cost, shared geometry, lib asserts, photosensitivity, empty frames, canvas allocation, accent budget), and exit 0 means green. From the stub pass onward, no step is done while the gate is red. On real scenes let it finish. Check 8 runs `tools/fixtures/asserts/*.js` only under `--fixtures` (each file calls `FILM.assert`); a film with no asserts passes as "no asserts". Check 9 fails more than three flashes in any one-second window. Check 10 warns on an empty frame or a tiny subject and does not fail the gate by itself. Check 11 fails when a canvas is allocated on the second of two full passes at scale 0.25 with the grain post off, and names the shot, the count, the total area and the first T. Peak live canvas area above about 16× the frame warns and does not fail. `--canvas-skip` skips it. Check 12 warns, and does not fail, when the theme's accent colour breaks `accent.budget` in `docs/theme.json` (`frames`: the longest run of consecutive frames it shows; `share`: the share of the film's frames it shows on; `area`: the most of one frame it covers); it reads the check 9 frames, so `--flash-skip` skips it too, and a theme with no budget passes.
