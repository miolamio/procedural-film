...
`node tools/check.cjs` is the gate: seven checks (media scan, determinism, source scan, timeline, draw, frame cost, shared geometry), and exit 0 means green. From the stub pass onward, no step is done while the gate is red. On real scenes it takes about a minute — let it finish.
