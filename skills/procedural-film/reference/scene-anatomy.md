...
- The medium is the timeline's `carrier`, not the scene: `crt` scanlines and screen, `vhs` chroma lag, tracking tears, head noise and the tape counter, `film` perforations, scratches, dust and weave, `xerox` toner grain, skips, dropout bands and the copy's jitter are drawn once per frame over the finished picture, transitions included. A scene never draws its own.
