...
- Optional per-shot `grade: { invert, warmth, fade, vignette, paperAge, tint, tintAmount, duotone, duotoneAmount }`. `warmth` is −1..1; `invert`, `fade`, `vignette`, `paperAge`, `tintAmount` and `duotoneAmount` are 0..1; `tint` is a `lib.pal` name, `duotone` is `[dark, light]`, two `lib.pal` names. `core` applies it after the shot draws and before the grain (`difference` / `saturation` / `multiply` / `screen` / `overlay`, one vignette gradient). `invert` goes first, so the rest grades the negative; 1 is the exact negative, and a cut into or out of it is one full-frame flash for check 9. `duotone` comes next: a gradient map of the frame's luminosity from `dark` (black) to `light` (white), exact on every channel where `light` is not darker than `dark` (a channel where it is stays at `dark`); `duotoneAmount` defaults to 1, 0 is the plate. Across `transitionIn` it interpolates from the outgoing grade to the incoming one. Omit `grade` and the frame is unchanged.
